...
class AnalysItemView(SelectableWidget):
    def _set_up(self, item: PeriodicityAnalysys):
        ...
        self.configure(padding=(5, 0))

        self.checkbox.pack(fill=X)
        self.checkbox.configure(text=item.name)

        ttk.Label(self, text=f"alg: {item.algorithm}").pack(fill=X)
        ttk.Label(self, text=f"ilość wzorców: {item.patterns_count}").pack(fill=X)
        self.__state_label = ttk.Label(self, text="przetwarzanie", foreground="red")
