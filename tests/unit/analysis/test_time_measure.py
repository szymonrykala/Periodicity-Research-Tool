...
def test_performance_results_data_initialization(
    data, patterns, expected_lengths, expected_texts
):
...
def test_env_not_stable_error():
    with pytest.raises(
        EnvNotStableError, match="Could not reach required deviation threshold"
    ):
...
@patch("sprt.analysis.time_measure.AlgorithmStore")
def test_time_measurement_run(
    mock_algorithm_store, mock_periodicity_analysis, mock_is_env_stable
):
    ...
    time_measurement = TimeMeasurement(algorithms, text_sets, patterns)
    with patch.object(
        time_measurement, "_TimeMeasurement__run_for_algorithm"
    ) as mock_run_alg:
